...
// Total usage
const totalRow = db.prepare("SELECT SUM(input_tokens) as inp, SUM(output_tokens) as out, COUNT(*) as count FROM events").get() as any;
...
const modelRows = db.prepare("SELECT model, provider, SUM(input_tokens) as inp, SUM(output_tokens) as out, COUNT(*) as count FROM events GROUP BY model ORDER BY SUM(input_tokens + output_tokens) DESC").all() as any[];
...
console.log("\n" + "─".repeat(60));
try {
  const compRow = db.prepare("SELECT AVG(ratio) as avgRatio, SUM(tokens_saved) as saved, COUNT(*) as count FROM compression_events").get() as any;
...
console.log("\n" + "─".repeat(60));
try {
  const budgetRow = db.prepare("SELECT * FROM budget_tracking ORDER BY date DESC LIMIT 1").get() as any;
  if (budgetRow) {
