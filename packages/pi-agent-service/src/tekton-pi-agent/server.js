...
import { configure, getServiceStatus, gannAnalyze, gannLevels, gannRange, gannPlanetary, gannS9, gannAngles, fibAnalyze, masterBridgeSignal, glmAsk, glmInterpretGann, tvExecuteTrade, tvClosePosition, tvTradeStatus, tvQuote, tvData, tvCheckConnection, getPositions, getTradeHistory, recordPosition, recordTrade, } from "./engine-bridge.js";
// ── MCP Server (stdio) ─────────────────────────────────────────────────
        ...
        try {
            ...
            switch (toolName) {
                case "pi_gann_analyze":
                    result = await gannAnalyze(args);
                    break;
                case "pi_gann_levels":
                    result = await gannLevels(args);
                    break;
                case "pi_fib_analyze":
                    result = await fibAnalyze(args);
                    break;
                case "pi_trade_signal":
                    result = await masterBridgeSignal(args);
                    break;
                case "pi_glm_ask":
                    result = await glmAsk(args);
                    break;
                case "pi_quote":
                    result = await tvQuote(args.symbol);
                    break;
                case "pi_data":
                    result = await tvData(args);
                    break;
                case "pi_execute_trade":
                    result = await tvExecuteTrade(args);
                    break;
                case "pi_positions":
                    result = getPositions();
                    break;
