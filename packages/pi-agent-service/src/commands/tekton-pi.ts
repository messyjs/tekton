...
async function check(): Promise<{ ok: boolean; data?: any }> {
  try {
...
async function api(path: string, method = "GET", body?: any): Promise<any> {
  const opts: RequestInit = { method, signal: AbortSignal.timeout(30000) };
  if (body) {
...
export const piCommand = {
  ...
  async handler(args: any, ctx: any, pi: any, piCtx: any): Promise<void> {
    ...
    switch (sub) {
      case "status": {
        ...
        if (!ok) { piCtx?.ui?.notify?.("❌ PI Agent sidecar not running. Start with: /tekton:pi start") ?? console.log("❌ Sidecar not running"); return; }
        const msg = [
      ...
      case "start": {
        ...
        if (existing.ok) { piCtx?.ui?.notify?.("✅ Already running on :7706") ?? console.log("Already running"); return; }
      ...
      case "stop": {
        try {
          ...
          if (process.platform === "win32") execSync(`taskkill /F /FI "WINDOWTITLE eq tekton-pi-agent*" 2>nul`);
          else execSync(`pkill -f "tekton-pi-agent" 2>/dev/null`);
          piCtx?.ui?.notify?.("✅ Stopped") ?? console.log("Stopped");
        } catch { piCtx?.ui?.notify?.("May not be running") ?? console.log("Not running?"); }
        return;
      }

      case "signal": {
        const symbol = rest[0]; const price = rest[1] ? parseFloat(rest[1]) : undefined;
        const timeframe = rest[2] ?? "4H"; const trend = rest[3] ?? "up";
        if (!symbol) { piCtx?.ui?.notify?.("Usage: /tekton:pi signal BTCUSD 104500 4H up") ?? console.log("Usage: signal BTCUSD 104500 4H up"); return; }
        try {
          let actualPrice = price;
          if (!actualPrice) { const q = await api(`/tv/quote/${symbol}`); actualPrice = q.price; }
          const pivots = [
          ...
          piCtx?.ui?.notify?.(msg) ?? console.log(msg);
        } catch (err: any) { piCtx?.ui?.notify?.(`❌ ${err.message}`) ?? console.error(err); }
        return;
      }

      case "gann": {
        if (rest[0] === "planetary") {
          try { const r = await api("/gann/planetary"); piCtx?.ui?.notify?.(JSON.stringify(r, null, 2)) ?? console.log(r); }
          catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
          return;
        }
      ...
      case "fib": { piCtx?.ui?.notify?.("Use /tekton:pi signal — Fib is included automatically") ?? console.log("Use signal"); return; }

      case "quote": {
        const symbol = rest[0];
        if (!symbol) { piCtx?.ui?.notify?.("Usage: /tekton:pi quote BTCUSD") ?? console.log("Usage: quote BTCUSD"); return; }
        try { const r = await api(`/tv/quote/${symbol}`); piCtx?.ui?.notify?.(`📊 ${r.symbol}: $${r.price}`) ?? console.log(`${r.symbol}: $${r.price}`); }
        catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
        return;
      }

      case "positions": {
        try { const p = await api("/positions"); piCtx?.ui?.notify?.(p.length ? `📐 ${p.length} positions` : "No positions") ?? console.log(p); }
        catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
        return;
      }

      case "history": {
        try { const h = await api("/trade-history"); piCtx?.ui?.notify?.(h.length ? `📜 ${h.length} trades` : "No history") ?? console.log(h); }
        catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
        return;
      }
      ...
      case "glm": {
        ...
        if (!question) { piCtx?.ui?.notify?.("Usage: /tekton:pi glm <question>") ?? console.log("Usage: glm <question>"); return; }
        try { const r = await api("/glm/ask", "POST", { question }); piCtx?.ui?.notify?.(`💡 ${r.answer}`) ?? console.log(r.answer); }
        catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
        return;
      }
      ...
      case "trade": {
        ...
        if (!direction || !symbol) { piCtx?.ui?.notify?.("Usage: /tekton:pi trade buy BTCUSD 104500 104311 104601") ?? console.log("Usage: trade buy BTCUSD 104500 104311 104601"); return; }
        try { const r = await api("/tv/execute", "POST", { direction, symbol, price: p, sl, tp }); piCtx?.ui?.notify?.(`✅ ${direction} ${symbol}`) ?? console.log(r); }
        catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
        return;
      }

      case "close": {
        try { const r = await api("/tv/close", "POST"); piCtx?.ui?.notify?.("✅ Closed") ?? console.log(r); }
        catch (e: any) { piCtx?.ui?.notify?.(`❌ ${e.message}`) ?? console.error(e); }
        return;
      }
