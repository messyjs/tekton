...
function makeValidBrief(overrides: Partial<ProductBrief> = {}): ProductBrief {
  return {
    id: "brief-test123",
    title: "Test Product",
...
describe("Brief Schema", () => {
  ...
  it("rejects invalid estimatedComplexity", () => {
    const brief = makeValidBrief({ estimatedComplexity: "super-hard" as any });
    const result = validateBrief(brief);
    expect(result.valid).toBe(false);
  });

  it("rejects invalid domain names", () => {
    const brief = makeValidBrief({ domains: ["invalid-domain" as any] });
    const result = validateBrief(brief);
    expect(result.valid).toBe(false);
  });
