...
describe("Creative Team", () => {
  ...
  it("generateBrief produces a valid ProductBrief", async () => {
    ...
    const brief = await team.generateBrief((transcript: string) => {
  ...
  it("custom response generator is used", async () => {
    const team = new CreativeTeam(config);
    const response = await team.getNextResponse(
      "Tell me about the UX flow",
      async (sys, user) => `Custom response for: ${user.slice(0, 20)}`,
    );
