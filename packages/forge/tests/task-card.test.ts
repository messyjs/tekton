...
describe("Task Card", () => {
  ...
  it("valid status transition: in-progress → completed", () => {
    const card = { ...createTaskCard("plan-1", "dev", "Task", "Desc"), status: "in-progress" as const };
  ...
  it("valid status transition: failed → in-progress (retry)", () => {
    const card = { ...createTaskCard("plan-1", "dev", "Task", "Desc"), status: "failed" as const };
    const updated = updateStatus(card, "in-progress");
    expect(updated.status).toBe("in-progress");
  });

  it("valid status transition: blocked → pending (unblocked)", () => {
    const card = { ...createTaskCard("plan-1", "dev", "Task", "Desc"), status: "blocked" as const };
  ...
  it("invalid transition throws: completed → in-progress", () => {
    const card = { ...createTaskCard("plan-1", "dev", "Task", "Desc"), status: "completed" as const };
  ...
  it("getNextReady returns tasks with met dependencies", () => {
    ...
    const cardB = { ...createTaskCard("plan-1", "dev", "B", "Desc B", [cardA.id]), status: "pending" as const };
    ...
    const completedA = { ...cardA, status: "completed" as const };
  ...
  it("getDependencyOrder produces valid topological sort", () => {
    ...
    const cardB = { ...createTaskCard("plan-1", "dev", "B", "After A", [cardA.id]), status: "pending" as const };
    const cardC = { ...createTaskCard("plan-1", "dev", "C", "After A", [cardA.id]), status: "pending" as const };
    const cardD = { ...createTaskCard("plan-1", "dev", "D", "After B and C", [cardB.id, cardC.id]), status: "pending" as const };
  ...
  it("getDependencyOrder detects cycles and throws", () => {
    const cardA = { ...createTaskCard("plan-1", "dev", "A", "After B", ["tc-b"]), status: "pending" as const };
    cardA.id = "tc-a";
    const cardB = { ...createTaskCard("plan-1", "dev", "B", "After A", ["tc-a"]), status: "pending" as const };
    cardB.id = "tc-b";
