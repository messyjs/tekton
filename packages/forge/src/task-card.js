...
// ── Create ─────────────────────────────────────────────────────────────────
export function createTaskCard(planId, role, title, description, deps = []) {
    return {
...
export function updateStatus(card, newStatus) {
    ...
    if (!allowed || !allowed.includes(newStatus)) {
        throw new Error(`Invalid status transition: ${card.status} → ${newStatus}. ` +
 ...
 */
export function getNextReady(cards) {
 ...
 */
export function getDependencyOrder(cards) {
