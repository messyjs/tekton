...
export function validateRole(role) {
    ...
    if (typeof role.sessionLimit !== "number" || role.sessionLimit <= 0)
        errors.push("sessionLimit must be a positive number");
    if (!role.systemPrompt.includes(".beta"))
        errors.push("systemPrompt must mention .beta file naming");
    return errors;
}
/**
 * Build a full system prompt for a role given a task.
 */
export function buildSystemPrompt(role, taskTitle, taskDescription) {
    return [
        role.systemPrompt,
        "",
        "## Current Assignment",
...
//# sourceMappingURL=base-role.js.map