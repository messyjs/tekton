...
import { getReady } from "./dependency-resolver.js";
...
export class ProductionManager {
     ...
     * 4. Return results
     */
    async executePlan(plan, projectDir, executor) {
        // 1. Copy project template
        ...
        while (completed.length + failed.length < taskCards.length && iteration < maxIterations) {
            ...
            if (ready.length === 0) {
                ...
                const remaining = taskCards.filter((c) => c.status !== "completed" && c.status !== "failed");
     ...
     */
    async executeTask(card, plan, executor) {
