 ...
 */
export function spawnProductionAgent(card, role, handoff) {
 ...
 */
function buildInstruction(card, role) {
    const parts = [
        role.systemPrompt,
        "",
        "## Your Task",
 ...
 */
function buildCuratedContext(card, handoff) {
    const parts = [];
 ...
 */
export function getDomainTopics(role) {
    ...
    if (role.tools) {
        ...
        if (toolStr.includes("juce") || toolStr.includes("audio"))
            allTopics.push("juce", "dsp", "audio");
        if (toolStr.includes("react") || toolStr.includes("frontend"))
            allTopics.push("react", "component");
    }
