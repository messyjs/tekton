...
function loadCriteria() {
    ...
    _config = JSON.parse(readFileSync(filePath, "utf-8"));
    return _config;
}
/**
 ...
 */
export async function evaluateBrief(brief, callLLM) {
...
async function llmEvaluate(brief, config, callLLM) {
...
function parseEvaluation(raw, config) {
    try {
        ...
        return heuristicEvaluate({ id: "", title: "", problemStatement: "", proposedSolution: "", technicalApproach: "", userStories: [], risks: [], estimatedComplexity: "medium", domains: ["web-app"], ideationTranscript: "", createdAt: Date.now(), revisionHistory: [] }, config);
    }
}
/** Heuristic evaluation when no LLM is available. */
function heuristicEvaluate(brief, config) {
    const scores = {};
