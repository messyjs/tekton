...
import { getTeamTemplate } from "../domain-registry.js";
 ...
 */
export async function generatePlan(brief, domains, callLLM) {
    // Load team templates
    ...
    try {
        ...
        throw new Error(`Generated plan has circular dependencies: ${e.message}`);
    }
...
function templateGenerateTasks(brief, teamTemplate) {
    ...
    for (const role of teamTemplate.roles) {
        ...
        const card = createTaskCard(planId, role.id, `${role.name}: ${brief.title}`, `Implement ${role.name} responsibilities for the ${brief.title} product. Brief context: ${brief.proposedSolution}`, deps);
    ...
    const integrationCard = createTaskCard(planId, "integrator", "Integration and Testing", `Integrate all components and verify the product works end-to-end`, allRoleIds);
...
async function llmGenerateTasks(brief, teamTemplate, callLLM) {
...
Generate the task breakdown.`;
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            const planId = `plan-${randomUUID().slice(0, 8)}`;
            return parsed.map((t) => {
                const card = createTaskCard(planId, t.role ?? "unknown", t.title ?? "Untitled task", t.description ?? "", t.dependencies ?? []);
