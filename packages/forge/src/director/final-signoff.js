 ...
 */
export async function finalSignoff(manifest, brief, callLLM) {
    const issues = [];
    // Check all artifacts are at "testing" or "release" status
    const incompleteArtifacts = manifest.artifacts.filter((a) => a.status !== "testing" && a.status !== "release");
...
function parseSignoffResponse(raw) {
    try {
...
//# sourceMappingURL=final-signoff.js.map