import { aggregateResults } from "./verdict.js";
const DEFAULT_QA_CONFIG = {
    sessionLimit: 15,
    maxQACycles: 2,
};
// ── Sub-agent definitions ──────────────────────────────────────────────────
export const UNIT_TESTER_ROLE = {
    id: "unit-tester",
    name: "Unit Tester",
...
export const INTEGRATION_TESTER_ROLE = {
...
export const REVIEW_AGENT_ROLE = {
    id: "review-agent",
    name: "Code Reviewer",
...
export class QAManager {
     ...
     */
    async runQAPipeline(projectDir, manifest) {
        const results = [];
        // 1. Domain validators
                ...
                if (validator) {
                    try {
                        ...
                        results.push({
                            ...
                            details: `Domain validator error: ${e.message}`,
                        });
                    }
                }
                else {
        ...
        // 2. Unit tests
        const unitResult = await this.runSubAgentOrMock(UNIT_TESTER_ROLE, `Run unit tests for all source files in ${projectDir}`);
        results.push(unitResult);
        // 3. Integration tests
        const integrationResult = await this.runSubAgentOrMock(INTEGRATION_TESTER_ROLE, `Build and run integration tests for ${projectDir}`);
        results.push(integrationResult);
        // 4. Code review
        const reviewResult = await this.runSubAgentOrMock(REVIEW_AGENT_ROLE, `Review all source files in ${projectDir}`);
    ...
    // ── Private helpers ───────────────────────────────────────────────
    async runSubAgentOrMock(role, context) {
        if (this.runSubAgent) {
...
//# sourceMappingURL=qa-manager.js.map