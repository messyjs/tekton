 ...
 * 5. Return new path
 */
export function promoteArtifact(projectDir, betaPath, manifest) {
    const fileName = basename(betaPath);
    // 1. Check QA signoffs
    const signoffs = manifest.qaSignoffs.filter(s => s.artifactPath === fileName || s.artifactPath === betaPath);
    if (signoffs.length === 0) {
        throw new Error(`Cannot promote "${fileName}" to release: no QA signoffs found. ` +
            `All artifacts must have at least one passing QA signoff before release.`);
    }
    const hasPassed = signoffs.some(s => s.passed);
    if (!hasPassed) {
        throw new Error(`Cannot promote "${fileName}" to release: no passing QA signoffs. ` +
    ...
    const updatedManifest = {
        ...manifest,
        artifacts: manifest.artifacts.map(a => a.path === betaPath || a.path === fileName
            ? { ...a, status: "release", lastModified: Date.now(), path: originalName }
            : a),
    };
 ...
 */
export function promoteAll(projectDir, manifest) {
    const promotedPaths = [];
    const skipped = [];
