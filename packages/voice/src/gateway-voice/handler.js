...
const DEFAULT_GATEWAY_VOICE_CONFIG = {
    telegramVoice: false,
    discordVoice: false,
...
export class GatewayVoiceHandler {
    ...
    sttManager;
    ttsManager;
    constructor(sttManager, ttsManager, config) {
    ...
    async generateVoiceReply(text, platform, channelId) {
        if (!this.ttsManager)
            return null;
        // Check if voice is enabled for this platform
        if (platform === "telegram" && !this.config.telegramVoice)
            return null;
        if (platform === "discord" && !this.config.discordVoice)
            return null;
        try {
    ...
    async convertToWav(audioData, fromFormat) {
    ...
    async convertForTelegram(audioPath) {
