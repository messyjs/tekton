...
import { browserNavigateTool, browserSnapshotTool, browserClickTool, browserTypeTool, browserPressTool, browserScrollTool, browserBackTool, browserConsoleTool, browserGetImagesTool, browserVisionTool, browserCdpTool, } from "./toolsets/browser/index.js";
...
import { piGannAnalyzeTool, piGannLevelsTool, piGannPlanetaryTool, piGannRangeTool, piFibAnalyzeTool, piTradeSignalTool, piGlmAskTool, piQuoteTool, piDataTool, piExecuteTradeTool, piPositionsTool, piTradeHistoryTool, piStatusTool, } from "./toolsets/pi/index.js";
import { rlListEnvironments, rlSelectEnvironment, rlGetCurrentConfig, rlEditConfig, rlStartTraining, rlStopTraining, rlCheckStatus, rlListRuns, rlGetResults, rlTestInference, } from "./toolsets/rl/index.js";
export function registerAllTools() {
    ...
    if (mcpDisconnectTool)
        registry.register(mcpDisconnectTool);
    if (appDiscoverTool)
        registry.register(appDiscoverTool);
    if (appConnectTool)
        registry.register(appConnectTool);
    if (appControlTool)
        registry.register(appControlTool);
    if (appSurfaceTool)
        registry.register(appSurfaceTool);
    if (appScreenshotTool)
        registry.register(appScreenshotTool);
    if (appLearnTool)
        registry.register(appLearnTool);
    if (appShortcutTool)
        registry.register(appShortcutTool);
    if (appTypeTool)
        registry.register(appTypeTool);
    if (appSendNoteTool)
        registry.register(appSendNoteTool);
    if (appSendCCTool)
        registry.register(appSendCCTool);
    if (appSendPatchTool)
        registry.register(appSendPatchTool);
    if (appSendOSCMessageTool)
        registry.register(appSendOSCMessageTool);
