...
function getManager(tektonHome) {
    if (!memoryManager) {
...
export const memoryTool = {
    name: "memory",
    toolset: "memory",
    description: "Save important information to persistent memory that survives across sessions. Actions: save, search, list, forget.",
...
export const sessionSearchTool = {
    ...
    async execute(params, context) {
        ...
        try {
            const results = searcher.search(params.query, params.limit ?? 5);
            if (results.length === 0) {
