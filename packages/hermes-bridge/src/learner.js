...
export class Learner {
    skillManager;
    userModel;
    constructor(skillManager, userModel) {
    ...
    extractSkill(context) {
        ...
        if (!procedure || procedure.steps.length < 2)
            return null;
     ...
     */
    refineSkill(existing, newApproach) {
     ...
     */
    isBetterApproach(existing, newEval) {
     ...
     */
    forceExtract(messages, description) {
        const procedure = this.extractProcedure(messages);
        const pitfalls = this.extractPitfalls(messages);
        const verification = this.extractVerification(messages);
        const body = this.buildSkillBody({
    ...
    extractTags(messages, taskDescription) {
        const tags = new Set();
    ...
    extractRequiredToolsets(messages) {
    ...
    inferCategory(description) {
        ...
        if (lower.includes("test") || lower.includes("spec"))
            return "testing";
        if (lower.includes("deploy") || lower.includes("infra") || lower.includes("config"))
