...
import type { CommandRegistration, CommandContext, ParsedArgs } from "../src/commands/types.js";
...
function createMockPiCtx() {
  ...
  return {
    ui: {
      notify: vi.fn((msg: string) => { messages.push(msg); }),
...
describe("Full Command Registry (all 26 commands)", () => {
  ...
  it("registers all commands with Pi extension API", () => {
    ...
    registry.registerAll(mockPi as any, ctx);
    ...
    const registeredNames = calls.map((call: [string, unknown]) => call[0]);
...
describe("Command Handlers", () => {
  ...
  let piCtx: ReturnType<typeof createMockPiCtx>;
  ...
  describe("/tekton (status dashboard)", () => {
    it("shows status dashboard with --json flag", async () => {
      ...
      const args: ParsedArgs = { subcommand: "", positional: [], flags: { json: true }, raw: "--json" };

      const mockPi = { registerCommand: vi.fn() } as any;
      await cmd.handler(args, ctx, mockPi, piCtx as any);
    ...
    it("shows formatted status without --json", async () => {
      const registry = createFullCommandRegistry();
      const cmd = registry.get("tekton")!;
      const args: ParsedArgs = { subcommand: "", positional: [], flags: {}, raw: "" };

      await cmd.handler(args, ctx, {} as any, piCtx as any);
  ...
  describe("/tekton:route", () => {
    ...
    it("changes route mode", async () => {
      const cmd = createFullCommandRegistry().get("tekton:route")!;
      const args: ParsedArgs = { subcommand: "deep", positional: [], flags: {}, raw: "deep" };
  ...
  describe("/tekton:learn", () => {
    ...
    it("pauses learning", async () => {
      const cmd = createFullCommandRegistry().get("tekton:learn")!;
      const args: ParsedArgs = { subcommand: "pause", positional: [], flags: {}, raw: "pause" };
    ...
    it("resumes learning", async () => {
      ...
      const cmd = createFullCommandRegistry().get("tekton:learn")!;
      const args: ParsedArgs = { subcommand: "resume", positional: [], flags: {}, raw: "resume" };
  ...
  describe("/tekton:personality", () => {
    it("sets overlay to a preset", async () => {
      ...
      const args: ParsedArgs = { subcommand: "set", positional: ["teacher"], flags: {}, raw: "set teacher" };
    ...
    it("clears overlay", async () => {
      ...
      const cmd = createFullCommandRegistry().get("tekton:personality")!;
      const args: ParsedArgs = { subcommand: "clear", positional: [], flags: {}, raw: "clear" };
    ...
    it("lists available presets", async () => {
      const cmd = createFullCommandRegistry().get("tekton:personality")!;
      const args: ParsedArgs = { subcommand: "list", positional: [], flags: {}, raw: "list" };
  ...
  describe("/tekton:memory", () => {
    it("adds memory entry", async () => {
      ...
      const args: ParsedArgs = { subcommand: "add", positional: ["test", "memory"], flags: {}, raw: "add test memory" };
    ...
    it("searches memory", async () => {
      ...
      const cmd = createFullCommandRegistry().get("tekton:memory")!;
      const args: ParsedArgs = { subcommand: "search", positional: ["hello"], flags: {}, raw: "search hello" };
    ...
    it("shows memory", async () => {
      ...
      const cmd = createFullCommandRegistry().get("tekton:memory")!;
      const args: ParsedArgs = { subcommand: "show", positional: [], flags: {}, raw: "show" };
  ...
  describe("/tekton:config", () => {
    ...
    it("shows config as JSON with --json flag", async () => {
      const cmd = createFullCommandRegistry().get("tekton:config")!;
      const args: ParsedArgs = { subcommand: "export", positional: [], flags: {}, raw: "export" };
  ...
  describe("/tekton:help", () => {
    ...
    it("shows help for specific command", async () => {
      const registry = createFullCommandRegistry();
      const cmd = registry.get("tekton:help")!;
      const args: ParsedArgs = { subcommand: "route", positional: [], flags: {}, raw: "route" };
  ...
  describe("train command (full implementation)", () => {
    ...
    it("shows stub message for voice status", async () => {
      ...
      const args: ParsedArgs = { subcommand: "status", positional: [], flags: {}, raw: "status" };
  ...
  describe("destructive action guards", () => {
    it("requires --force for memory forget", async () => {
      const cmd = createFullCommandRegistry().get("tekton:memory")!;
      const args: ParsedArgs = { subcommand: "forget", positional: [], flags: {}, raw: "forget" };
    ...
    it("clears memory with --force", async () => {
      ...
      const cmd = createFullCommandRegistry().get("tekton:memory")!;
      const args: ParsedArgs = { subcommand: "forget", positional: [], flags: { force: true }, raw: "forget --force" };
    ...
    it("requires --force for config reset", async () => {
      const cmd = createFullCommandRegistry().get("tekton:config")!;
      const args: ParsedArgs = { subcommand: "reset", positional: [], flags: {}, raw: "reset" };
