...
import { SoulManager, PersonalityManager, MemoryManager } from "@tekton/core";
// ── Mock helpers ─────────────────────────────────────────────────────
function createMockSoul(content) {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "tekton-test-"));
    const soul = new SoulManager(tmpDir);
    soul.setSoul(content);
    return soul;
}
function createMockPersonality(soul) {
    return new PersonalityManager(soul);
}
function createMockMemory(tmpDir) {
