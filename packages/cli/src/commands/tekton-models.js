...
function handleStatus(args, ctx, piCtx) {
    ...
    if (Object.keys(byModel).length > 0) {
        const modelRows = Object.entries(byModel).map(([model, data]) => [model, `${data.calls} calls`, `$${data.cost.toFixed(4)}`, `${data.inputTokens + data.outputTokens} tok`]);
...
function handleProviders(args, piCtx) {
    ...
    const rows = Object.entries(PROVIDERS).map(([id, p]) => {
        ...
        const hasKey = p.apiKeyEnv ? `$${p.apiKeyEnv}` : (p.local ? 'local' : '—');
        const costIn = p.costPer1KInput ? `$${p.costPer1KInput}/1K` : "—";
        return [`  ${id}`, p.name, `${modelCount}`, hasKey, costIn];
    });
...
function handleCost(args, ctx, piCtx) {
    const costTracker = ctx.modelRouter.getCostTracker();
    ...
    if (Object.keys(report.byModel).length > 0) {
        const modelRows = Object.entries(report.byModel).map(([model, data]) => [`  ${model}`, `${data.calls}`, `$${data.cost.toFixed(4)}`, `${data.inputTokens + data.outputTokens}`]);
        output += "\n\nBy model:\n" + formatTable(["Model", "Calls", "Cost", "Tokens"], modelRows);
    }
    // Cost by provider
    if (Object.keys(report.byProvider).length > 0) {
        const provRows = Object.entries(report.byProvider).map(([prov, data]) => [`  ${prov}`, `${data.calls}`, `$${data.cost.toFixed(4)}`]);
        output += "\n\nBy provider:\n" + formatTable(["Provider", "Calls", "Cost"], provRows);
    }
    piCtx.ui.notify(output);
}
//# sourceMappingURL=tekton-models.js.map