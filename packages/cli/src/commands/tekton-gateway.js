import { formatTable } from "./types.js";
...
async function handleStart(args, ctx, piCtx) {
    ...
    try {
        ...
        const platforms = Object.entries(status.platforms)
            .map(([name, ps]) => `  ${ps.connected ? "✓" : "✗"} ${name}`)
            .join("\n");
        const apiPort = status.platforms["api-server"] ? "7700" : "-";
        const whPort = status.platforms["webhook"] ? "7701" : "-";
        piCtx.ui.notify(`🟢 Gateway started\n${platforms}\n` +
...
async function handlePlatforms(piCtx) {
    if (!gatewayInstance) {
        piCtx.ui.notify("Gateway is not running.");
        return;
    }
    ...
    piCtx.ui.notify(formatTable(["Platform", "Status", "In", "Out", "Last Error"], rows));
}
async function handleSessions(ctx, piCtx) {
    if (!gatewayInstance) {
        piCtx.ui.notify("Gateway is not running.");
        return;
    }
    ...
    piCtx.ui.notify(formatTable(["Platform", "User ID", "Name", "Msgs", "Model", "Last Active"], rows));
}
//# sourceMappingURL=tekton-gateway.js.map