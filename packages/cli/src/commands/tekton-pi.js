        ...
        async handler(args, ctx, pi, piCtx) {
            ...
            switch (sub) {
                ...
                case "signal": {
                    ...
                    if (!symbol) {
                        piCtx.ui.notify("Usage: /tekton:pi signal BTCUSD 104500 4H up");
                        return;
                    }
                    ...
                    try {
                        ...
                        const result = await defaultPiAgent.tradeSignal({ symbol, price: actualPrice, pivots, timeframe, trend: trend });
                        const lines = [
                        ...
                        if (result.crossEngineConfluence?.length)
                            lines.push(`   Confluence: ${result.crossEngineConfluence.length} zones`);
                        if (result.planetaryWarning)
                            lines.push(`   ⚠️ Planetary Warning: ${result.planetaryWarning}`);
                ...
                case "quote": {
                    const symbol = rest[0];
                    if (!symbol) {
                        piCtx.ui.notify("Usage: /tekton:pi quote BTCUSD");
                        return;
                    }
                    try {
                        ...
                        piCtx.ui.notify(`❌ Quote failed: ${err.message}`);
                    }
                    return;
                }
                // ── Positions ───────────────────────────────────────────────
                case "positions": {
                    ...
                    if (!positions.length) {
                        piCtx.ui.notify("No open positions");
                    }
                    else {
                ...
                case "history": {
                    ...
                    if (!history.length) {
                        piCtx.ui.notify("No trade history");
                    }
                    else {
                ...
                case "glm": {
                    ...
                    if (!question) {
                        piCtx.ui.notify("Usage: /tekton:pi glm <question>");
                        return;
                    }
                    try {
                        ...
                        piCtx.ui.notify(`❌ GLM failed: ${err.message}`);
                    }
                    return;
                }
                // ── Trade ──────────────────────────────────────────────────
                case "trade": {
                    ...
                    if (!direction || !symbol) {
                        piCtx.ui.notify("Usage: /tekton:pi trade buy BTCUSD 104500 104311 104601");
                        return;
                    }
                    try {
                        const result = await defaultPiAgent.tvExecuteTrade({ direction: direction, symbol, price: p, sl, tp });
                        piCtx.ui.notify(`✅ Trade: ${direction} ${symbol}`);
                    }
                    catch (err) {
                        piCtx.ui.notify(`❌ Trade failed: ${err.message}`);
                    }
                    return;
                }
                // ── Close ───────────────────────────────────────────────────
                case "close": {
                    try {
                        ...
                        piCtx.ui.notify(`❌ Close failed: ${err.message}`);
                    }
                    return;
                }
                ...
                case "http-stop": {
                    try {
                        ...
                        if (process.platform === "win32") {
                            execSync(`for /f "tokens=5" %a in ('netstat -ano ^| findstr :7706 ^| findstr LISTENING') do taskkill /F /PID %a 2>nul`, { stdio: "ignore" });
                        }
                        else {
                            execSync(`lsof -ti:7706 | xargs kill 2>/dev/null`, { stdio: "ignore" });
                        }
