...
export function createAbletonCommand() {
    return {
        name: "tekton:ableton",
...
async function handlePlay(piCtx) {
    ...
    if (!ok) {
        piCtx.ui.notify("❌ Ableton sidecar not running. Start with: /tekton:ableton start");
        return;
    }
    try {
...
async function handleStop(piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ Ableton sidecar not running");
        return;
    }
    try {
...
async function handleTracks(piCtx) {
    ...
    try {
        ...
        if (!data.tracks || data.tracks.length === 0) {
            piCtx.ui.notify("No tracks found (AbletonOSC may not be connected)");
            return;
        }
...
async function handleStart(piCtx) {
    const { ok } = await checkHealth();
    if (ok) {
        piCtx.ui.notify("✅ Ableton sidecar already running on port 7703");
        return;
    }
    const { installed } = checkInstalled();
    if (!installed) {
        piCtx.ui.notify("❌ Not installed. Run: pip install tekton-ableton");
        return;
    }
    ...
    try {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            if ((await checkHealth()).ok) {
                piCtx.ui.notify(`✅ Ableton sidecar started on port ${PORT}`);
                return;
            }
        }
...
async function handleStopSvc(piCtx) {
    if (sidecarProcess) {
        ...
        sidecarProcess = null;
        piCtx.ui.notify("✅ Ableton sidecar stopped");
    }
    else {
        piCtx.ui.notify("⚠️ No local sidecar process found");
    }
}
//# sourceMappingURL=tekton-ableton.js.map