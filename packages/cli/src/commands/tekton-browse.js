...
export function createBrowseCommand() {
    return {
        ...
        handler: async (args, ctx, pi, piCtx) => {
            ...
            switch (sub) {
                case "status":
                    await handleBrowseStatus(piCtx);
                    break;
                case "task":
                    await handleBrowseTask(args, piCtx);
                    break;
                case "start":
                    await handleBrowseStart(piCtx);
                    break;
                case "stop":
                    await handleBrowseStop(piCtx);
                    break;
...
async function handleBrowseStatus(piCtx) {
...
async function handleBrowseTask(args, piCtx) {
    const taskDescription = args.positional.slice(1).join(" ");
    if (!taskDescription) {
        piCtx.ui.notify("Usage: /tekton:browse task <description>");
        return;
    }
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("Sidecar not running. Start with: /tekton:browse start");
        return;
    }
...
async function handleBrowseStart(piCtx) {
    const { ok } = await checkHealth();
    if (ok) {
        piCtx.ui.notify("Browser-use sidecar already running on port 7702");
        return;
    }
    const { installed } = checkInstalled();
    if (!installed) {
        piCtx.ui.notify("Not installed. Run: pip install tekton-browser-use");
        return;
    }
    ...
    try {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            if ((await checkHealth()).ok) {
                piCtx.ui.notify(`Sidecar started on port ${BROWSE_PORT}`);
                return;
            }
        }
...
async function handleBrowseStop(piCtx) {
    if (sidecarProcess) {
        sidecarProcess.kill("SIGTERM");
        sidecarProcess = null;
        piCtx.ui.notify("Sidecar stopped");
    }
    else {
        piCtx.ui.notify("No local sidecar process found");
    }
}
//# sourceMappingURL=tekton-browse.js.map