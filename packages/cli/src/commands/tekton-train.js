    ...
    return {
        ...
        handler: async (args, _ctx, _pi, piCtx) => {
            ...
            switch (sub) {
                ...
                default:
                    piCtx.ui.notify("🧠 Training Management\n\n" +
                        "Subcommands:\n" +
...
async function handleConfig(args, piCtx) {
    ...
    if (hasJsonFlag(args)) {
        ...
        piCtx.ui.notify("🧠 Training Configuration\n\n" +
...
async function handleStart(args, piCtx) {
    const prompt = args.positional.join(" ");
    if (!prompt) {
    ...
    if (hasJsonFlag(args)) {
        ...
        piCtx.ui.notify(`🚀 Training job started\n\n` +
            `  Job ID:  ${job.id}\n` +
            `  Method:  ${job.method}\n` +
...
async function handleStatus(args, piCtx) {
    const orch = await getOrchestrator();
    ...
    if (jobId) {
        ...
        if (hasJsonFlag(args)) {
            ...
            piCtx.ui.notify(`🧠 Training Status: ${job.name}\n\n` +
                `  Job ID:     ${job.id}\n` +
                `  Method:     ${job.method}\n` +
                `  Status:     ${job.status}\n` +
                `  Progress:   ${progress}%\n` +
        ...
        if (hasJsonFlag(args)) {
            ...
            piCtx.ui.notify("🧠 Training Orchestrator\n\n" +
...
async function handleStop(args, piCtx) {
    const jobId = args.positional[0];
    if (!jobId) {
...
async function handleEval(args, piCtx) {
    const jobId = args.positional[0];
    if (!jobId) {
    ...
    try {
        ...
        piCtx.ui.notify(`📊 Evaluation Results\n\n` +
            `  Benchmark: ${result.benchmark}\n` +
            Object.entries(result.scores).map(([task, score]) => `  ${task}: ${(score * 100).toFixed(1)}%`).join("\n") +
...
async function handleExport(args, piCtx) {
    const jobId = args.positional[0];
    ...
    try {
        const exportDir = await checkpoints.exportCheckpoint(best.id, format);
...
async function handleList(args, piCtx) {
    const orch = await getOrchestrator();
    ...
    if (hasJsonFlag(args)) {
        ...
        const lines = jobs.map((j) => `  ${j.id.slice(0, 8)}  ${j.method.padEnd(15)} ${j.status.padEnd(12)} ${(j.progress * 100).toFixed(0).padStart(3)}%  ${j.config.baseModel}`);
        piCtx.ui.notify("🧠 Training Jobs\n\n" +
            "  ID       Method          Status       Progress  Model\n" +
            "  " + "─".repeat(75) + "\n" +
            lines.join("\n"));
    }
}
async function handleLogs(args, piCtx) {
    const jobId = args.positional[0];
    if (!jobId) {
    ...
    if (hasJsonFlag(args)) {
        ...
        piCtx.ui.notify(`📊 Training Metrics: ${jobId}\n\n` +
...
async function handleGPU(args, piCtx) {
    const orch = await getOrchestrator();
...
//# sourceMappingURL=tekton-train.js.map