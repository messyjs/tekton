...
export function createLearnCommand() {
    return {
        name: "tekton:learn",
        ...
        handler: async (args, ctx, _pi, piCtx) => {
            ...
            switch (sub) {
                ...
                default: {
                    const status = ctx.hermesBridge.getStatus();
                    piCtx.ui.notify(`Learning: ${status.isPaused ? "paused" : "active"}\n` +
...
function status_is_paused(ctx) {
    return ctx.hermesBridge.getStatus().isPaused;
}
//# sourceMappingURL=tekton-learn.js.map