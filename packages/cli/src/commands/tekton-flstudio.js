...
export function createFLStudioCommand() {
    return {
        ...
        handler: async (args, ctx, pi, piCtx) => {
            ...
            switch (sub) {
                case "status":
                    ...
                    break;
                case "play":
                    await handlePlay(piCtx);
                    break;
                case "stop":
                    await handleStop(piCtx);
                    break;
                case "tempo":
                    await handleTempo(args, piCtx);
                    break;
                case "channels":
                    await handleChannels(piCtx);
                    break;
                case "tracks":
                    await handleTracks(piCtx);
                    break;
                case "plugins":
                    await handlePlugins(args, piCtx);
                    break;
                case "piano":
                    await handlePianoRoll(piCtx);
                    break;
                case "start":
                    await handleStart(piCtx);
                    break;
                case "stop_svc":
                    await handleStopSvc(piCtx);
                    break;
...
async function handleStatus(piCtx) {
...
async function handlePlay(piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running. Start with: /tekton:flstudio start");
        return;
    }
    try {
...
async function handleStop(piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running");
        return;
    }
    try {
...
async function handleTempo(args, piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running");
        return;
    ...
    if (isNaN(tempo) || tempo < 20 || tempo > 999) {
        piCtx.ui.notify("❌ Invalid tempo. Must be 20-999 BPM.");
        return;
    }
    try {
...
async function handleChannels(piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running");
        return;
    ...
    try {
        ...
        if (!data.channels || data.channels.length === 0) {
            piCtx.ui.notify("No channels found (FL Studio bridge may not be connected)");
            return;
        }
...
async function handleTracks(piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running");
        return;
    ...
    try {
        ...
        if (!data.tracks || data.tracks.length === 0) {
            piCtx.ui.notify("No mixer tracks found");
            return;
        }
...
async function handlePlugins(args, piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running");
        return;
    ...
    try {
        ...
        if (!data.params || data.params.length === 0) {
            piCtx.ui.notify(`No plugin on channel ${channelId}`);
            return;
        }
...
async function handlePianoRoll(piCtx) {
    const { ok } = await checkHealth();
    if (!ok) {
        piCtx.ui.notify("❌ FL Studio sidecar not running");
        return;
    ...
    try {
        ...
        const noteCount = result.noteCount ?? result.notes?.length ?? 0;
        const success = result.success ?? true;
        if (!success) {
            piCtx.ui.notify("❌ Piano roll state unavailable (need flpianoroll in FL Studio)");
            return;
        }
...
async function handleStart(piCtx) {
    const { ok } = await checkHealth();
    if (ok) {
        piCtx.ui.notify("✅ FL Studio sidecar already running on port 7704");
        return;
    }
    const { installed } = checkInstalled();
    if (!installed) {
        piCtx.ui.notify("❌ Not installed. Run: pip install tekton-flstudio");
        return;
    }
    ...
    try {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            if ((await checkHealth()).ok) {
                piCtx.ui.notify(`✅ FL Studio sidecar started on port ${PORT}`);
                return;
            }
        }
        ...
        piCtx.ui.notify(`❌ Failed: ${err instanceof Error ? err.message : String(err)}`);
    }
}
async function handleStopSvc(piCtx) {
    if (sidecarProcess) {
        sidecarProcess.kill("SIGTERM");
        sidecarProcess = null;
        piCtx.ui.notify("✅ FL Studio sidecar stopped");
    }
    else {
        piCtx.ui.notify("⚠️ No local sidecar process found");
    }
}
//# sourceMappingURL=tekton-flstudio.js.map