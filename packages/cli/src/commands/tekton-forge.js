...
function setForgeEnabled(enabled) {
        ...
        handler: async (args, ctx, _pi, piCtx) => {
            ...
            if (subcommand === "new") {
                ...
                try {
                    ...
                    piCtx.ui.notify(`❌ Error creating project: ${e.message}`);
                    return;
                }
            }
            ...
            if (subcommand === "status") {
                ...
                piCtx.ui.notify(projects.map(p => `${p.id}  ${p.phase}  ${p.title ?? ""}  ${p.error ? "⚠️ " + p.error : ""}`).join("\n"));
                return;
            }
            ...
            if (subcommand === "resume") {
                ...
                try {
                    ...
                    piCtx.ui.notify(`Resumed project ${projectId} at phase "${state.currentPhase}".\n${state.error ? `Error: ${state.error}` : ""}`);
                    return;
                }
                catch (e) {
                    piCtx.ui.notify(`❌ Error resuming project: ${e.message}`);
                    return;
                }
            }
            // list — list projects
            if (subcommand === "list") {
                ...
                piCtx.ui.notify(projects.map(p => `${p.id}  ${p.phase}  ${p.title ?? ""}`).join("\n"));
                return;
            }
            ...
            if (subcommand === "check") {
                ...
                try {
                    const result = await forge.checkDomain(domain);
                    const lines = [
                        `Domain: ${domain}`,
                    ...
                    piCtx.ui.notify(`❌ Preflight check error: ${e.message}`);
                    return;
                }
            }
...
//# sourceMappingURL=tekton-forge.js.map