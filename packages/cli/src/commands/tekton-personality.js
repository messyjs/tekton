    ...
    return {
        ...
        handler: async (args, ctx, _pi, piCtx) => {
            ...
            switch (sub) {
                case "list": {
                    ...
                    if (hasJsonFlag(args)) {
                        ...
                        for (const name of presets) {
                            const desc = PERSONALITY_PRESETS[name].split("\n")[0]?.replace(/^# Overlay: /, "") ?? "";
                ...
                default: {
                    const hasOverlay = ctx.personality.hasOverlay();
                    piCtx.ui.notify(`👤 Personality: ${hasOverlay ? "overlay active" : "default (SOUL.md)"}\n\n` +
...
//# sourceMappingURL=tekton-personality.js.map