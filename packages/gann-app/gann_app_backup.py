...
def fetch_market_data(ticker: str, period: str = "6mo") -> dict:
    """Fetch OHLCV data and compute pivots."""
    try:
        t = yf.Ticker(ticker)
        ...
        # Find pivot high and low with timestamps
...
async def get_tickers():
    ...
    return {"tickers": [{"symbol": k, "name": v} for k, v in POPULAR_TICKERS.items()]}

@app.get("/api/market/{ticker}")
async def get_market_data(ticker: str, period: str = "6mo"):
...
def analyze_casper(market: dict, ticker: str) -> dict:
    """Jayson Casper analysis: Market Cipher, VWAP, Order Flow, Fib."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    fib_range = high - low
    golden_pocket_low = high - fib_range * 0.618
    golden_pocket_high = high - fib_range * 0.786
    
    # Market Cipher B signals (simulated from price action)
    bias = "BULLISH" if change_pct > 0 else "BEARISH"
    if abs(change_pct) < 0.5:
        bias = "NEUTRAL"
    
    # VWAP proximity
    vwap_approx = (high + low + price) / 3  # Typical VWAP approximation
    vwap_distance_pct = ((price - vwap_approx) / vwap_approx) * 100
    vwap_signal = "ABOVE_VWAP_BULLISH" if price > vwap_approx else "BELOW_VWAP_BEARISH"
    if abs(vwap_distance_pct) < 0.2:
        vwap_signal = "AT_VWAP_DECISION"
    
    # Divergence check (using RSI proxy from change)
    div_signal = "HIDDEN_BULLISH_DIV" if change_pct > 1.5 else "HIDDEN_BEARISH_DIV" if change_pct < -1.5 else "NO_CLEAR_DIV"
    
    # Order flow proxy (volume-weighted)
    vol = market.get("volume", 0)
    cvd_signal = "BULLISH_CVD" if change_pct > 0 else "BEARISH_CVD"
    
    # Build key levels
    key_levels = [
        {"name": "Fib 0.618 (Golden Pocket Low)", "price": round(golden_pocket_low, 2), "type": "support"},
        {"name": "Fib 0.786 (Golden Pocket High)", "price": round(golden_pocket_high, 2), "type": "support"},
        ...
        {"name": "VWAP Approximation", "price": round(vwap_approx, 2), "type": "decision"},
        {"name": "Pivot High", "price": round(high, 2), "type": "resistance"},
        {"name": "Pivot Low", "price": round(low, 2), "type": "support"},
    ]
    
    # Score (0-100)
    score = 50
    if price > vwap_approx: score += 10
    if price > golden_pocket_high: score += 10
    if change_pct > 1: score += 10
    if change_pct > 3: score += 5
    if div_signal.startswith("BULLISH"): score += 10
    ...
    return {
        ...
        "signals": {
            ...
            "golden_pocket": f"{round(golden_pocket_low,2)} - {round(golden_pocket_high,2)}",
            "divergence": div_signal,
            "cvd": cvd_signal,
            "bias": bias,
        },
        "keyLevels": key_levels,
        "methods": ["Market Cipher B Divergences", "VWAP Acceptance/Rejection", "Fibonacci Golden Pocket",
                     "Order Flow (CVD/Delta)", "Money Flow Cross", "Single Print Fill"],
        "market": market,
        "timeframe": "4H",
        "summary": f"Casper Analysis: {bias} bias (score {score}/100). VWAP {vwap_signal.replace('_', ' ')}. Golden pocket {round(golden_pocket_low,2)}-{round(golden_pocket_high,2)}. Divergence: {div_signal}",
    }

def analyze_rumors(market: dict, ticker: str) -> dict:
...
def analyze_geo(market: dict, ticker: str) -> dict:
    """Trader Geo analysis: USDT.D, PO3, Order Blocks."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    
    # For crypto, analyze USDT.D proxy
    # When crypto rises, USDT.D typically falls (inverse correlation)
    ...
    ob_ranges = [
        {"name": "Bullish OB (Last Sell Before Rally)", "price": round(low + (high - low) * 0.2, 2), "type": "support"},
        {"name": "Bearish OB (Last Buy Before Drop)", "price": round(high - (high - low) * 0.2, 2), "type": "resistance"},
    ]
    
    # PO3 session analysis (simplified)
    sessions = {
        "Asian": {"phase": "ACCUMULATION", "range": f"{round(low, 2)} - {round(low + (high-low)*0.33, 2)}"},
        "London": {"phase": "MANIPULATION", "range": f"{round(low + (high-low)*0.33, 2)} - {round(low + (high-low)*0.66, 2)}"},
        "New York": {"phase": "DISTRIBUTION", "range": f"{round(low + (high-low)*0.66, 2)} - {round(high, 2)}"},
    }
    
    # Key levels
    key_levels = [
        {"name": "Bearish OB", "price": ob_ranges[1]["price"], "type": "resistance"},
        {"name": "Bullish OB", "price": ob_ranges[0]["price"], "type": "support"},
        {"name": "Pivot High (Liquidity)", "price": round(high, 2), "type": "resistance"},
        {"name": "Pivot Low (Liquidity)", "price": round(low, 2), "type": "support"},
        {"name": "Equilibrium (50%)", "price": round((high + low) / 2, 2), "type": "pivot"},
    ]
    ...
    return {
        "engine": "geo",
        "ticker": ticker,
        "bias": crypto_implication,
        "score": score,
        "currentPrice": price,
        "signals": {
            "usdt_d_direction": usdt_direction,
            "usdt_d_proxy": round(usdt_d_proxy, 2),
            "crypto_implication": crypto_implication,
            "po3_sessions": sessions,
            "order_blocks": {"bullish": ob_ranges[0]["price"], "bearish": ob_ranges[1]["price"]},
        },
        "keyLevels": key_levels,
        "methods": ["USDT.D Dominance", "PO3 (Power of 3)", "Order Blocks", "Multi-TF Analysis", "Session Trading"],
        "market": market,
        "timeframe": "4H",
        "summary": f"Geo Analysis: USDT.D {usdt_direction} = {crypto_implication.replace('_', ' ')}. PO3: {sessions['Asian']['phase']}/{sessions['London']['phase']}/{sessions['New York']['phase']}. OBs: Bullish {ob_ranges[0]['price']}, Bearish {ob_ranges[1]['price']}",
    }

def analyze_ict(market: dict, ticker: str) -> dict:
    """ICT/Smart Money analysis: Order Blocks, FVG, Liquidity Sweeps."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    # Fair Value Gaps
    fvg_bullish = round(low + range_size * 0.382, 2)
    fvg_bearish = round(high - range_size * 0.382, 2)
    
    # Order Blocks
    bullish_ob = round(low + range_size * 0.15, 2)
    bearish_ob = round(high - range_size * 0.15, 2)
    ...
    # Market structure
    structure = "BULLISH_BOS" if change_pct > 0 and price > (high + low) / 2 else \
               "BEARISH_BOS" if change_pct < 0 and price < (high + low) / 2 else "RANGING"
    
    # Optimal Trade Entry zones
    ote_long = round(bullish_ob, 2)
    ote_short = round(bearish_ob, 2)
    
    key_levels = [
        {"name": "Buy-Side Liquidity", "price": buy_side_liquidity, "type": "resistance"},
        {"name": "Bearish OB (OTE Short)", "price": bearish_ob, "type": "resistance"},
        {"name": "Bearish FVG", "price": fvg_bearish, "type": "gap"},
        {"name": "Equilibrium", "price": round((high + low) / 2, 2), "type": "pivot"},
        {"name": "Bullish FVG", "price": fvg_bullish, "type": "gap"},
        {"name": "Bullish OB (OTE Long)", "price": bullish_ob, "type": "support"},
    ...
    return {
        ...
        "keyLevels": key_levels,
        "methods": ["Order Blocks", "Fair Value Gaps", "Liquidity Sweeps", "Optimal Trade Entry", "Market Structure", "Breaker Blocks"],
        "market": market,
        "timeframe": "4H",
        "summary": f"ICT Analysis: {structure.replace('_', ' ')}. Bullish OB {bullish_ob}, Bearish OB {bearish_ob}. BSL {buy_side_liquidity}, SSL {sell_side_liquidity}. FVG zones {fvg_bullish}/{fvg_bearish}",
    }

def analyze_mj(market: dict, ticker: str) -> dict:
...
def analyze_franky(market: dict, ticker: str) -> dict:
    """Frankie Candles analysis: Candle patterns, S/R, multi-TF confluence."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    # Key S/R levels
    levels = [round(low + range_size * r, 2) for r in [0, 0.145, 0.236, 0.382, 0.5, 0.618, 0.786, 1.0]]
    # Find nearest S/R
    dists = [(abs(price - l), l, i) for i, l in enumerate(levels)]
    nearest = min(dists, key=lambda x: x[0])
    sr_position = "AT_SUPPORT" if nearest[2] < 3 else "AT_RESISTANCE" if nearest[2] > 5 else "MID_RANGE"
    
    # Candle pattern proxy from price action
    body = abs(change_pct)
    if body > 2: pattern = "STRONG_CANDLE" if change_pct > 0 else "STRONG_BEAR_CANDLE"
    elif body > 1: pattern = "MODERATE_CANDLE"
    else: pattern = "DOJI/INDECISION"
    
    # Multi-TF confluence (simplified)
    mtf = "BULLISH_CONFLUENCE" if change_pct > 0.5 else "BEARISH_CONFLUENCE" if change_pct < -0.5 else "NO_CONFLUENCE"
    
    key_levels = [
        {"name": "Major Support", "price": round(low + range_size * 0.145, 2), "type": "support"},
        {"name": "Key S/R Low", "price": round(low + range_size * 0.236, 2), "type": "support"},
        {"name": "Fib 38.2%", "price": round(low + range_size * 0.382, 2), "type": "pivot"},
        {"name": "Fib 50%", "price": round(low + range_size * 0.5, 2), "type": "decision"},
        {"name": "Fib 61.8%", "price": round(low + range_size * 0.618, 2), "type": "pivot"},
        {"name": "Key S/R High", "price": round(low + range_size * 0.786, 2), "type": "resistance"},
        {"name": "Major Resistance", "price": round(high, 2), "type": "resistance"},
    ]
    
    score = 50
    if sr_position == "AT_SUPPORT" and change_pct > 0: score += 15
    if mtf == "BULLISH_CONFLUENCE": score += 10
    if pattern == "STRONG_CANDLE": score += 5
    score = max(0, min(100, score))
    bias = f"{sr_position} {pattern.replace('_', ' ')}"
    
    return {
        "engine": "franky", "ticker": ticker, "bias": bias.replace('_', ' '), "score": score,
        "currentPrice": price,
        "signals": {"sr_position": sr_position, "candle_pattern": pattern, "mtf_confluence": mtf, "nearest_sr": nearest[1]},
        "keyLevels": key_levels,
        "methods": ["Candle Pattern Recognition", "Key S/R Levels", "Multi-TF Confluence", "Clean Chart Reading"],
        "market": market, "timeframe": "4H",
        "summary": f"Frankie Candles: {sr_position.replace('_', ' ')}, {pattern.replace('_', ' ')}. MTF: {mtf.replace('_', ' ')}. Nearest S/R: {nearest[1]}",
    }

def analyze_cryptoface(market: dict, ticker: str) -> dict:
    """CryptoFace analysis: On-chain, order flow, liquidation levels."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    # On-chain proxy (from market metrics)
    vol = market.get("volume", 0)
    vol_trend = "HIGH_VOLUME" if vol > 0 else "NORMAL_VOLUME"
    
    # Liquidation level estimation (psychological levels)
    if price > 1000:  # BTC-like
        liq_levels = [round(price / 1000) * 1000 + i * 500 for i in range(-4, 5)]
        liq_levels = [l for l in liq_levels if low * 0.9 < l < high * 1.1]
    else:
        liq_levels = [round(price / 100) * 100 + i * 50 for i in range(-3, 4)]
        liq_levels = [l for l in liq_levels if low * 0.9 < l < high * 1.1]
    
    # CVD proxy
    cvd = "BULLISH_CVD" if change_pct > 0 else "BEARISH_CVD"
    
    # Exchange flow proxy
    if change_pct > 1.5: flow = "HOT_OUTFLOW_BULLISH"
    elif change_pct < -1.5: flow = "HOT_INFLOW_BEARISH"
    else: flow = "NEUTRAL_FLOW"
    
    key_levels = [
        {"name": "Liquidation Cluster Below", "price": round(price * 0.97, 2), "type": "support"},
        {"name": "Liquidation Cluster Above", "price": round(price * 1.03, 2), "type": "resistance"},
        {"name": "Whale Accumulation Zone", "price": round(low + range_size * 0.236, 2), "type": "support"},
        {"name": "Pivot Low (Exchange Flow)", "price": round(low, 2), "type": "support"},
        {"name": "Pivot High (Liquidations)", "price": round(high, 2), "type": "resistance"},
    ]
    
    score = 50
    if cvd == "BULLISH_CVD": score += 10
    if flow.endswith("BULLISH"): score += 15
    if change_pct > 1: score += 5
    score = max(0, min(100, score))
    bias = f"{flow.replace('_', ' ')} {cvd.replace('_', ' ')}"
    
    return {
        "engine": "cryptoface", "ticker": ticker, "bias": bias, "score": score,
        "currentPrice": price,
        "signals": {"cvd": cvd, "exchange_flow": flow, "volume_trend": vol_trend, "liquidation_levels": liq_levels[:5]},
        "keyLevels": key_levels,
        "methods": ["On-Chain Analysis", "Order Flow Reading", "Liquidation Levels", "Exchange Flow", "CVD"],
        "market": market, "timeframe": "4H",
        "summary": f"CryptoFace: {flow.replace('_', ' ')}, {cvd.replace('_', ' ')}. Volume: {vol_trend.replace('_', ' ')}",
    }

def analyze_buffett(market: dict, ticker: str) -> dict:
    """Warren Buffett analysis: Value investing, intrinsic value, margin of safety."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    # Intrinsic value estimation (simplified - compare price to range center)
    range_center = (high + low) / 2
    margin_of_safety = ((range_center - price) / price) * 100 if price else 0
    if price < low * 1.1: mos_rating = "DEEP_VALUE"  
    elif price < range_center: mos_rating = "VALUE_ZONE"
    elif price < high * 0.9: mos_rating = "FAIR_VALUE"
    else: mos_rating = "OVERVALUED"
    
    # Economic moat proxy (from volatility - lower = more stable = stronger moat)
    moat = "WIDE_MOAT" if volatility < 0.3 else "NARROW_MOAT" if volatility < 0.5 else "NO_MOAT"
    
    # Circle of competence check
    is_crypto = "USD" in ticker.upper() and any(c in ticker for c in ["BTC", "ETH", "SOL"])
    competence = "OUTSIDE_CIRCLE" if is_crypto else "WITHIN_CIRCLE"
    
    # Owner earnings proxy
    pe_proxy = 25 if not is_crypto else 0  # Can't P/E crypto
    
    key_levels = [
        {"name": "Deep Value Zone (Low + 10%)", "price": round(low * 1.1, 2), "type": "support"},
        {"name": "Value Zone (Range Center)", "price": round(range_center, 2), "type": "decision"},
        {"name": "Fair Value (Range 75%)", "price": round(low + (high - low) * 0.75, 2), "type": "resistance"},
        {"name": "Overvalued Zone", "price": round(high * 0.95, 2), "type": "resistance"},
    ]
    
    score = 50
    if mos_rating in ["DEEP_VALUE", "VALUE_ZONE"]: score += 20
    if moat == "WIDE_MOAT": score += 10
    if competence == "WITHIN_CIRCLE": score += 10
    score = max(0, min(100, score))
    ...
    return {
        ...
        "currentPrice": price,
        "signals": {"margin_of_safety": f"{margin_of_safety:.1f}%", "mos_rating": mos_rating, "economic_moat": moat, 
                     "circle_of_competence": competence, "volatility_regime": "LOW" if volatility < 0.3 else "HIGH"},
        "keyLevels": key_levels,
        "methods": ["Intrinsic Value", "Margin of Safety", "Economic Moats", "Circle of Competence", "Owner Earnings"],
        "market": market, "timeframe": "4H",
        "summary": f"Buffett: {mos_rating.replace('_', ' ')}, {moat.replace('_', ' ')}. MOS: {margin_of_safety:.1f}%. {competence.replace('_', ' ')}",
    }

def analyze_quant(market: dict, ticker: str) -> dict:
    """Aaron/QuantCrawler analysis: Quantitative, algorithmic, statistical edge."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    # Statistical edge estimation
    sharpe_proxy = change_pct / (volatility * 100) if volatility > 0 else 0
    edge = "POSITIVE_EDGE" if sharpe_proxy > 0.02 else "NEGATIVE_EDGE" if sharpe_proxy < -0.02 else "NO_EDGE"
    
    # Walk-forward proxy (trend strength)
    trend_strength = abs(change_pct) / (volatility * 100) if volatility > 0 else 0
    regime = "TRENDING" if trend_strength > 0.5 else "MEAN_REVERTING" if trend_strength < 0.2 else "RANDOM_WALK"
    
    # Expected value calculation
    ev = change_pct * 0.6  # Simplified EV
    ev_label = "POSITIVE_EV" if ev > 0.3 else "NEGATIVE_EV" if ev < -0.3 else "NEUTRAL_EV"
    
    # Monte Carlo proxy (range of outcomes)
    mc_up = round(price * (1 + volatility * 0.5), 2)
    mc_down = round(price * (1 - volatility * 0.5), 2)
    mc_median = round(price * (1 + change_pct / 200), 2)
    ...
    key_levels = [
        ...
        {"name": "Stat Support", "price": round(low + range_size * 0.236, 2), "type": "support"},
        {"name": "Stat Resistance", "price": round(high - range_size * 0.236, 2), "type": "resistance"},
    ]
    ...
    return {
        ...
        "currentPrice": price,
        "signals": {"statistical_edge": edge, "regime": regime, "sharpe_proxy": round(sharpe_proxy, 4),
                     "expected_value": f"{ev:.2f}%", "mc_range": [mc_down, mc_median, mc_up]},
        "keyLevels": key_levels,
        "methods": ["Quantitative Strategy", "Statistical Edge", "Walk-Forward", "Monte Carlo", "Regime Detection"],
        "market": market, "timeframe": "4H",
        "summary": f"QuantCrawler: {edge.replace('_', ' ')}, {regime.replace('_', ' ')}. EV: {ev:.2f}%. MC Range: {mc_down}-{mc_up}",
    }

def analyze_tori(market: dict, ticker: str) -> dict:
    """Tori Trades: Action/Safety trendline system, candle close confirmation, losses-as-fees."""
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    change_pct = market.get("changePercent", 0)
    ...
    # Action Line / Safety Line system (Tori's core strategy)
    # Action line = trendline that was CROSSED (entry signal)
    # Safety line = opposing trendline (keeps you in the trade)
    # When price breaks down trendline -> LONG (action line crossed)
    # When price breaks up trendline -> SHORT (action line crossed)
    # Reverse position when safety line breaks
    action_line = round(low + range_size * 0.382, 2)  # Where trendline breaks
    safety_line = round(low + range_size * 0.618, 2)  # Opposing trendline
    
    # Candle close confirmation (Tori's #1 rule)
    candle_confirmed = abs(change_pct) > 0.3  # Significant candle close
    if price > action_line and change_pct > 0:
        trend_signal = "LONG_ON_BREAK"
        safety = safety_line
        stop = round(safety - (safety * 0.002), 2)
    elif price < safety_line and change_pct < 0:
        trend_signal = "SHORT_ON_BREAK"
        safety = action_line
        stop = round(safety + (safety * 0.002), 2)
    else:
        trend_signal = "WAIT_FOR_BREAK"  # In between lines, sit on hands
        safety = action_line
        stop = round(price * 0.998, 2)
    
    # 5-min momentum proxy
    ...
    # Risk management (prop firm style: 1-3% max)
    max_dd = 3.0
    risk_per = min(2, max_dd * 0.3)
    stop_distance = round(price * risk_per / 100, 2)
    
    # R:R calculation (target 2:1 minimum, let runners run)
    rr_ratio = 2.0
    target = round(price + stop_distance * rr_ratio if trend_signal.endswith("BULLISH") or "LONG" in trend_signal else price - stop_distance * rr_ratio, 2)
    
    key_levels = [
        {"name": "Scalp Target", "price": target, "type": "resistance"},
        {"name": "Action Line (Crossed)", "price": action_line, "type": "decision"},
        {"name": "Entry (Current)", "price": round(price, 2), "type": "entry"},
        {"name": "Safety Line (Stop)", "price": safety, "type": "support"},
        {"name": "Stop Loss", "price": stop, "type": "support"},
    ]
    
    score = 50
    if trend_signal == "LONG_ON_BREAK" and candle_confirmed: score += 20
    elif trend_signal == "SHORT_ON_BREAK" and candle_confirmed: score += 20
    ...
    return {
        ...
        "currentPrice": price,
        "signals": {"trend_signal": trend_signal, "momentum": momentum, "action_line": action_line, "safety_line": safety,
                     "candle_confirmed": candle_confirmed, "risk_per_trade": f"{risk_per:.1f}%",
                     "stop": stop, "target": target, "rr_ratio": rr_ratio},
        "keyLevels": key_levels,
        "methods": ["Trendline Break & Reverse", "Action/Safety Lines", "Candle Close Confirmation", "Losses as Fees Philosophy", "Prop Firm Risk (1-3%)", "5m Futures Scalping"],
        "market": market, "timeframe": "5m",
        "summary": f"Tori Trades: {trend_signal.replace('_', ' ')}, {momentum}. Action: {action_line}, Safety: {safety_line}. Stop: {stop}, Target: {target} ({rr_ratio}:1 R:R)",
    }

def analyze_dtr(market: dict, ticker: str) -> dict:
    """Day Trading Radio: 4-stage stochastic RSI with quad rotation and Holy Grail."""
    import math
    price = market.get("currentPrice", 0)
    high = market.get("pivotHigh", 0)
    low = market.get("pivotLow", 0)
    ...
    # 4 Stochastic RSI bands simulation (9-3, 14-3, 44-3, 60-10)
    # These represent momentum speed layers - fastest to slowest
    fast_k, fast_d = 9, 3     # Fast: 1m/2m equivalent
    mid1_k, mid1_d = 14, 3    # Mid-fast: 5m equivalent
    mid2_k, mid2_d = 44, 3    # Mid-slow: 15m equivalent
    slow_k, slow_d = 60, 10   # Slow: 1H equivalent
    
    # Simulate stochastic positions from price action
    # Using a proxy: normalized position in range = stoch-like reading
    price_norm = (price - low) / range_size if range_size > 0 else 0.5
    
    # Each band reads momentum at different speeds
    # Fast reacts quickly, slow is smoothed
    fast_stoch = max(0, min(100, price_norm * 100 + change_pct * 8))  # Very responsive
    mid1_stoch = max(0, min(100, price_norm * 100 + change_pct * 4))  # Moderate
    mid2_stoch = max(0, min(100, price_norm * 100 + change_pct * 2))  # Slow
    slow_stoch = max(0, min(100, price_norm * 100 + change_pct * 1))  # Very slow
    
    # OB/OS thresholds (DTR uses 20/80)
    ob_zone = 80
    os_zone = 20
    
    # Quad Rotation check (all 4 in OS or OB zone simultaneously)
    all_oversold = all(s <= os_zone for s in [fast_stoch, mid1_stoch, mid2_stoch, slow_stoch])
    all_overbought = all(s >= ob_zone for s in [fast_stoch, mid1_stoch, mid2_stoch, slow_stoch])
    
    # Holy Grail setup: Quad rotation OS + divergence + trendline confluence
    holy_grail = all_oversold and change_pct < -0.5  # OS + selling pressure = reversal imminent
    
    # Coil detection: 3-4 rotations of fast stoch into OS before breakout
    coil_detected = fast_stoch <= os_zone and mid1_stoch <= 35 and slow_stoch <= 40
    quad_rotation = "QUAD_OS_BUY" if all_oversold else "QUAD_OB_SELL" if all_overbought else "NO_QUAD"
    
    # Divergence proxy (fast shows OS but price not making new low)
    fast_os_not_new_low = fast_stoch <= os_zone and change_pct < 0  # Bullish div potential
    fast_ob_not_new_high = fast_stoch >= ob_zone and change_pct > 0  # Bearish div potential
    
    # Trend alignment using 20/50/200 EMAs (DTR uses these)
    ema20_above = change_pct > 0.3  # Short-term trend
    ema50_above = change_pct > 0  # Medium trend
    ema200_above = price > low + range_size * 0.7  # Long-term trend
    
    # VWAP (DTR uses VWAP for bounces and as support/resistance)
    vwap = (high + low + price) / 3
    at_vwap = abs(price - vwap) < range_size * 0.05
    above_vwap = price > vwap
    
    # Flag pattern proxy: strong move + shallow pullback
    flag_bullish = change_pct > 0.5 and fast_stoch > 70  # Strong trend + fast OB
    flag_bearish = change_pct < -0.5 and fast_stoch < 30  # Strong drop + fast OS
    ...
    # Score (Holy Grail is the highest)
    score = 50
    if holy_grail: score = 95
    elif all_oversold: score += 25
    elif all_overbought: score += 20
    if coil_detected: score += 10
    if fast_os_not_new_low: score += 10
    ...
    return {
        ...
        "currentPrice": price,
        "signals": {
            "fast_stoch_9_3": round(fast_stoch, 1),
            "mid1_stoch_14_3": round(mid1_stoch, 1),
            "mid2_stoch_44_3": round(mid2_stoch, 1),
            "slow_stoch_60_10": round(slow_stoch, 1),
            "quad_rotation": quad_rotation,
        ...
        "keyLevels": key_levels,
        "methods": ["4-Stage Stoch RSI (9/3/14/3/44/3/60/10)", "Quad Rotation", "Holy Grail", "Coil Detection", "Divergence", "Trendline Confirmation"],
        "market": market, "timeframe": "1m",
        "summary": f"DTR: {signal_type.replace('_', ' ')}. Fast={fast_stoch:.0f} Mid={mid1_stoch:.0f} Slow={slow_stoch:.0f}. Quad={quad_rotation}. Holy Grail={'YES' if holy_grail else 'no'}",
    }

def analyze_reece(market: dict, ticker: str) -> dict:
    ...
    # ── EMA Simulation (from EMA Cross Matrix) ──
    # Fast/medium/slow EMAs derived from price data
...
async def analyze_gann(request: dict):
    ...
    if engine_id == "casper":
        ...
        return run_gann_engine("run_gann_v2.js", [
...
async def chat(request: dict):
    ...
    # Try primary model, then fall back to local models
    # For local models, truncate context to fit their smaller context windows
    ...
    for model in models_to_try:
        try:
            ...
            body = json.dumps({
                ...
                "messages": use_msgs,
                "stream": False,
                "options": {"temperature": 0.3, "num_predict": 4096 if not is_local else 1024},
            }).encode()
...
async def get_candles(ticker: str, period: str = "3mo", interval: str = "1h"):
    ...
    try:
        ...
        if hist.empty:
            return {"candles": [], "error": f"No data for {ticker}"}
        candles = []
        ...
        return {"candles": candles, "ticker": ticker, "interval": interval}
    except Exception as e:
...
if __name__ == "__main__":
    ...
    print(f"\n  W.D. Gann Prediction Engine: http://127.0.0.1:{port}")
    print(f"  API: /api/analyze, /api/chat, /api/market/{{ticker}}")
