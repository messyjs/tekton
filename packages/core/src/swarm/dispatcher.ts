...
import { SwarmRosterManager, getSwarmRoster } from "./roster.js";
...
export class SwarmDispatcher {
  private roster: SwarmRosterManager;
  private missions: Map<string, SwarmMission> = new Map();
  private checkpoints: Map<string, SwarmCheckpoint[]> = new Map();
  private strategy: DispatchStrategy;
  private roundRobinIndex: number = 0;
  private eventListeners: Array<(event: SwarmEvent) => void> = [];

  constructor(roster: SwarmRosterManager, strategy: DispatchStrategy = "auto") {
    this.roster = roster;
  ...
  createMission(intent: string, autoAssign: boolean = true): SwarmMission {
  ...
  getActiveMissions(): SwarmMission[] {
    return [...this.missions.values()].filter(m =>
      m.state === "assigned" || m.state === "running" || m.state === "reviewing"
    );
  }
  ...
  private assignToWorker(mission: SwarmMission, worker: WorkerConfig, request: DispatchRequest): SwarmBrief[] {
    const brief = this.roster.createBrief({
      workerId: worker.id,
      project: mission.id,
      goal: request.intent,
  ...
  private findWorkerForRole(role: WorkerRole): WorkerConfig | null {
  ...
  private routeToReviewer(checkpoint: SwarmCheckpoint): void {
  ...
  private routeHandoff(checkpoint: SwarmCheckpoint): void {
    // Just mark the current worker as idle and let the next assignment happen
    this.roster.markIdle(checkpoint.workerId);
  }

  /** Generate goal description for a role */
  private generateGoalForRole(intent: string, role: WorkerRole): string {
    switch (role) {
  ...
  private generateDeliverables(role: WorkerRole): string[] {
    switch (role) {
  ...
  private generateProof(role: WorkerRole): string[] {
    return ["Build passes", "Tests pass"];
  }

  /** Find the mission containing a brief */
  private findMissionForBrief(briefId: string): SwarmMission | null {
    return [...this.missions.values()].find(m =>
      m.briefs.some(b => b.id === briefId)
    ) ?? null;
  }
...
let _dispatcher: SwarmDispatcher | null = null;

export function getSwarmDispatcher(): SwarmDispatcher {
  if (!_dispatcher) {
