...
export const RequireProof: ValidationRule = {
  name: "require-proof",
  states: ["DONE", "NEEDS_REVIEW", "HANDOFF"],
  validate(cp: SwarmCheckpoint): string[] {
    const issues: string[] = [];
...
export const RequireFilesChanged: ValidationRule = {
...
export const RequireBlockerDetail: ValidationRule = {
...
export const NotAdjectives: ValidationRule = {
...
export class SwarmCheckpointValidator {
  ...
  validate(checkpoint: SwarmCheckpoint): { valid: boolean; issues: string[] } {
...
export function requiresApproval(action: string): boolean {
  return (GATEKEEPER_ACTIONS as readonly string[]).includes(action);
}
