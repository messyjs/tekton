 ...
 */
export async function scanCDPPorts(ports) {
 ...
 */
export async function getCDPTargets(port) {
    try {
 ...
 */
export async function scanOSCPorts(ports) {
 ...
 */
export function identifyApp(process, knownApps = DEFAULT_KNOWN_APPS) {
...
export async function scanMIDIPorts() {
    try {
 ...
 */
export function determineProtocol(process, cdpEndpoints, oscEndpoints, midiEndpoints = [], knownApps = DEFAULT_KNOWN_APPS) {
    ...
    if (identified) {
        ...
        if (pattern.preferredProtocol === "midi") {
            ...
            const midiMatch = midiEndpoints.find(m => m.name.toLowerCase().includes(process.name.toLowerCase()));
            if (midiMatch || midiEndpoints.length > 0) {
                return {
                    layer: "midi",
                    midiPort: midiMatch?.port ?? 0,
                    midiPortName: midiMatch?.name,
                    midiChannel: pattern.midiChannel ?? 0,
                };
            }
        ...
        if (pattern.preferredProtocol === "cdp") {
            const cdp = cdpEndpoints.find(c => pattern.debugPort ? c.port === pattern.debugPort : true);
            if (cdp) {
        ...
        if (pattern.preferredProtocol === "osc") {
            const osc = oscEndpoints.find(o => pattern.oscPort ? o.port === pattern.oscPort : o.active);
            if (osc) {
...
export async function discoverApps(config) {
    ...
    const seen = new Set();
    const apps = [];
    const cdpUsed = new Set();
    for (const proc of processes) {
        // Skip tiny/background processes
        if (proc.memoryMB < 1 && !proc.windowTitle)
            continue;
