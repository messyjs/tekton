 ...
 */
async function powershell(script, timeout = 15000) {
...
export async function listWindows() {
  ...
  `;
    try {
        const result = await powershellJSON(script, 10000);
...
export async function findWindow(query) {
    ...
    return windows.find(w => w.title.toLowerCase().includes(lower) ||
        w.processName.toLowerCase().includes(lower)) ?? null;
}
/**
 ...
 */
export async function getControls(windowTitle) {
    const script = `
    Add-Type -AssemblyName UIAutomationClient
  ...
  `;
    try {
        const raw = await powershellJSON(script, 20000);
 ...
 */
export async function sendShortcut(keys, windowTitle) {
    try {
        // Focus the window if specified
        if (windowTitle) {
            const win = await findWindow(windowTitle);
            if (win)
                await focusWindow(win.handle);
        }
 ...
 */
export async function typeText(text, windowTitle) {
    try {
        if (windowTitle) {
            const win = await findWindow(windowTitle);
            if (win)
 ...
 */
export async function clickAt(x, y, windowTitle) {
    try {
        if (windowTitle) {
            const win = await findWindow(windowTitle);
            if (win)
 ...
 */
export async function selectMenu(windowTitle, menuPath) {
    try {
        // Send Alt to activate menu bar, then type the menu path
        if (menuPath.length === 0)
            return { success: false, layer: "uia", action: "menu", error: "Empty menu path" };
        const win = await findWindow(windowTitle);
        if (win)
            await focusWindow(win.handle);
...
export class UIADriver {
    ...
    async click(selector) {
        ...
        const target = controls.find(c => c.id === selector || c.label.includes(selector) || c.selector === selector);
        if (target?.bounds) {
...
//# sourceMappingURL=protocol-uia.js.map