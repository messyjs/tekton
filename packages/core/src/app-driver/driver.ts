...
import { DEFAULT_APP_DRIVER_CONFIG, DEFAULT_KNOWN_APPS } from "./types.js";
import {
  discoverApps,
  scanProcesses,
  scanCDPPorts,
...
import type { UIWindow } from "./protocol-uia.js";
import {
  UIADriver,
  findWindow,
  focusWindow,
  listWindows,
  getControls,
  takeScreenshot as uiaScreenshot,
...
import { MIDIDriver, CC_NUMBERS, MASSIVE_CC_MAP, SYNTH_CC_MAPS, noteToMidi, percentToMidi, midiToPercent } from "./protocol-midi.js";
import { OSCDriver, ABLETON_OSC_ADDRESSES, FL_STUDIO_OSC_ADDRESSES, REAPER_OSC_ADDRESSES, getOSCAddressMap } from "./protocol-osc.js";
...
export class ConnectedApp {
  ...
  private uiaDriver: UIADriver | null = null;
  private midiDriver: MIDIDriver | null = null;
  private oscDriver: OSCDriver | null = null;
  private _connected = false;
  private _surface: SurfaceControl[] = [];
  ...
  get appName(): string { return this.info.process.name; }
  get pid(): number { return this.info.process.pid; }
  get protocol(): ProtocolLayer { return this.info.protocol; }
  get connected(): boolean { return this._connected; }
  get surface(): SurfaceControl[] { return this._surface; }
  ...
  private async tryProtocol(proto: ProtocolLayer, action: ControlAction): Promise<ControlResult> {
    switch (proto) {
      case "midi":
        return this.executeMIDI(action);
      case "cdp":
        if (this.cdpDriver) return this.executeCDP(action);
        return { success: false, layer: "cdp", action: action.type, error: "CDP not connected" };
      case "uia":
        if (this.uiaDriver) return this.executeUIA(action);
  ...
  private async executeMIDI(action: ControlAction): Promise<ControlResult> {
    if (!this.midiDriver) {
  ...
  private async executeRealOSC(action: ControlAction): Promise<ControlResult> {
    if (!this.oscDriver) {
    ...
    switch (action.type) {
      case "osc_message": {
        ...
        let args: any[] = [];
        if (action.value) {
          try { args = JSON.parse(action.value); } catch { args = [parseFloat(action.value) || action.value]; }
        }
  ...
  private getAppOSCMap(): Record<string, string> {
  ...
  /** Execute via CDP */
  private async executeCDP(action: ControlAction): Promise<ControlResult> {
    if (!this.cdpDriver) return { success: false, layer: "cdp", action: action.type, error: "No CDP driver" };
  ...
  private async executeUIA(action: ControlAction): Promise<ControlResult> {
    if (!this.uiaDriver) return { success: false, layer: "uia", action: action.type, error: "No UIA driver" };
  ...
  /** Execute via Hotkey */
  private async executeHotkey(action: ControlAction): Promise<ControlResult> {
    ...
    switch (action.type) {
      ...
      case "shortcut":
        if (title) return sendHotkeyToWindow(title, action.value ?? action.target ?? "");
        return sendHotkey(action.value ?? action.target ?? "");

      case "menu":
        if (title) await focusWindowByTitle(title);
        await new Promise(r => setTimeout(r, 100));
  ...
  /** Screenshot fallback */
  private async executeScreenshot(action: ControlAction): Promise<ControlResult> {
    return uiaScreenshot(this.info.process.windowTitle || undefined);
  }

  /** Learn the app's control surface */
  async learnSurface(): Promise<SurfaceMap> {
...
export class AppDriver {
  readonly config: AppDriverConfig;
  private apps: Map<string, ConnectedApp> = new Map();
  private lastDiscovery: DiscoveredApp[] = [];
  ...
  async connect(appQuery: string): Promise<ConnectedApp | null> {
    ...
    const match = apps.find(a =>
      a.process.name.toLowerCase().includes(lower) ||
  ...
  listConnected(): Array<{ name: string; protocol: string; connected: boolean }> {
  ...
  async quickControl(appName: string, action: ControlAction): Promise<ControlResult> {
  ...
  async screenshot(appName?: string): Promise<ControlResult> {
    if (appName) {
      const app = this.get(appName) ?? await this.connect(appName);
      if (app) return app.execute({ type: "screenshot" });
    }
...
function getProtocolCascade(preferred: ProtocolLayer): ProtocolLayer[] {
  const cascades: Record<ProtocolLayer, ProtocolLayer[]> = {
...
function isDangerousAction(action: ControlAction): boolean {
  const dangerousTypes: ControlAction["type"][] = ["custom"];
  const dangerousTargets = ["delete", "remove", "format", "reset", "clear"];
  const dangerousValues = ["delete", "format", "reset", "clear", "rm "];
  if (dangerousTypes.includes(action.type)) return true;
  if (action.target && dangerousTargets.some(t => action.target!.toLowerCase().includes(t))) return true;
  if (action.value && dangerousValues.some(v => action.value!.toLowerCase().includes(v))) return true;
  return false;
}

function combineModifiers(modifiers: string[], key: string): string {
...
function extractRegions(controls: SurfaceControl[]): SurfaceMap["regions"] {
  const regions: SurfaceMap["regions"] = [];
...
let defaultDriver: AppDriver | null = null;

export function getAppDriver(config?: Partial<AppDriverConfig>): AppDriver {
  if (!defaultDriver) {
