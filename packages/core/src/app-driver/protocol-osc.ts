...
export interface OSCMessage {
  address: string;     // e.g., "/live/play"
  args?: OSCValue[];   // Arguments (int, float, string, blob)
}

export interface OSCBundle {
  timetag: number;     // NTP timetag (64-bit)
...
export const ABLETON_OSC_ADDRESSES: Record<string, string> = {
  // Transport
  play: "/live/play",
...
export const FL_STUDIO_OSC_ADDRESSES: Record<string, string> = {
  // Transport
  play: "/flstudio/play",
  stop: "/flstudio/stop",
...
export const REAPER_OSC_ADDRESSES: Record<string, string> = {
  play: "/transport/play",
  stop: "/transport/stop",
    ...
    for (const arg of args) {
      if (typeof arg === "number") {
        if (Number.isInteger(arg)) {
          ...
          buf.writeInt32BE(arg as number, 0);
          parts.push(buf);
        } else {
          // 32-bit big-endian float
          const buf = Buffer.alloc(4);
          buf.writeFloatBE(arg as number, 0);
          parts.push(buf);
        }
      } else if (typeof arg === "string") {
 ...
 */
export function encodeOSCBundle(timetag: number, elements: Array<OSCMessage | OSCBundle>): Buffer {
  const parts: Buffer[] = [];
 ...
 */
export function decodeOSCMessage(data: Buffer): { address: string; args: OSCValue[] } {
  let offset = 0;
...
export class OSCDriver extends EventEmitter {
  ...
  private config: OSCDriverConfig;
  ...
  get bound(): boolean { return this._bound; }
   ...
   */
  async send(host: string, port: number, address: string, args?: OSCValue[]): Promise<void> {
    if (!this.socket) await this.init();
    if (!this.socket) throw new Error("OSC socket not initialized");
   ...
   */
  async sendDefault(address: string, args?: OSCValue[]): Promise<void> {
   ...
   */
  async sendBundle(host: string, port: number, timetag: number, elements: OSCMessage[]): Promise<void> {
    if (!this.socket) await this.init();
    if (!this.socket) throw new Error("OSC socket not initialized");
  ...
  async abletonPlay(host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/play");
  }

  /** Stop Ableton playback */
  async abletonStop(host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/stop");
  }

  /** Set Ableton tempo */
  async abletonSetTempo(tempo: number, host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/tempo", [tempo]);
  }

  /** Set track volume (0.0 to 1.0) */
  async abletonSetVolume(track: number, volume: number, host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/track/set/volume", [track, volume]);
  }

  /** Set track pan (-1.0 to 1.0) */
  async abletonSetPan(track: number, pan: number, host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/track/set/pan", [track, pan]);
  }

  /** Arm a track for recording */
  async abletonArmTrack(track: number, armed: boolean, host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/track/set/arm", [track, armed ? 1 : 0]);
  }

  /** Fire a clip */
  async abletonFireClip(track: number, clip: number, host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7703, "/live/clip/fire", [track, clip]);
  }

  /** Stop a clip */
  async abletonStopClip(track: number, clip: number, host?: string, port?: number): Promise<void> {
  ...
  async reaperPlay(host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7001, "/transport/play");
  }

  /** Stop Reaper playback */
  async reaperStop(host?: string, port?: number): Promise<void> {
    return this.send(host ?? "127.0.0.1", port ?? 7001, "/transport/stop");
  }

  /** Set Reaper track volume (0.0 to 1.0) */
  async reaperSetVolume(track: number, volume: number, host?: string, port?: number): Promise<void> {
  ...
  onMessage(handler: (msg: { address: string; args: OSCValue[] }, rinfo: RemoteInfo) => void): void {
    this.on("message", handler);
  }

  /** Wait for a specific OSC address pattern */
  async waitFor(address: string, timeoutMs: number = 5000): Promise<OSCValue[]> {
    return new Promise((resolve, reject) => {
      ...
      const handler = (msg: { address: string; args: OSCValue[] }) => {
   ...
   */
  async probe(host: string, port: number, pingAddress: string = "/ping"): Promise<boolean> {
    try {
      if (!this.socket) await this.init();
...
function readPaddedString(data: Buffer, offset: number): string {
  let end = offset;
...
export function toNTPTimetag(date: Date = new Date()): number {
...
export function getOSCAddressMap(appName: string): Record<string, string> {
