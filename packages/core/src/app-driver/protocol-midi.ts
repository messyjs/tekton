...
export interface MIDIMessage {
  status: number;   // Status byte (channel + message type)
  data1: number;    // First data byte (note/cc/program)
  data2: number;    // Second data byte (velocity/value)
  timestamp?: number;
}
...
export interface NoteOffParams {
  channel: number;
  note: number;
  velocity?: number;  // 0-127, default 0
}
...
export interface NRPNParams {
  channel: number;
  msb: number;       // 0-127 (parameter MSB)
  lsb: number;       // 0-127 (parameter LSB)
...
export const CC_NUMBERS: Record<string, number> = {
...
export const MASSIVE_CC_MAP: Record<string, number> = {
  // Oscillators
  osc1Pitch: 1,
  osc1Amp: 2,
  osc1Wavetable: 3,
...
export const SYNTH_CC_MAPS: Record<string, Record<string, number>> = {
...
export class MIDIDriver extends EventEmitter {
  private output: any = null;  // midi.Output instance
  private input: any = null;    // midi.Input instance (for feedback)
  private _portName: string = "";
  private _portNumber: number = -1;
  private _connected = false;
  private _virtualPort = false;
  private _channel: number = 0;
  private _noteTimers: Map<string, NodeJS.Timeout> = new Map();

  /** List available MIDI output ports */
  async listOutputPorts(): Promise<Array<{ port: number; name: string }>> {
    try {
  ...
  async listInputPorts(): Promise<Array<{ port: number; name: string }>> {
    try {
      const { Output, Input } = require("midi");
   ...
   */
  async connect(portNameOrNumber?: string | number, channel: number = 0): Promise<boolean> {
    try {
      const { Output, Input } = require("midi");
      ...
      if (typeof portNameOrNumber === "number") {
        ...
        const match = ports.find(p =>
          p.name.toLowerCase().includes(portNameOrNumber.toLowerCase())
        );
        if (match) {
   ...
   */
  async createVirtualPort(name: string = "Tekton MIDI Out"): Promise<boolean> {
    try {
      const { Output, Input } = require("midi");
      this.output = new Output();
  ...
  get portName(): string { return this._portName; }
  ...
  sendNoteOff(channel: number, note: number, velocity: number = 0): void {
    this.sendRaw(0x80 | (channel & 0x0F), note & 0x7F, velocity & 0x7F);
  }

  /** Send a Note On with automatic Note Off after duration */
  async sendNote(channel: number, note: number, velocity: number = 100, durationMs: number = 500): Promise<void> {
  ...
  sendParameter(synthName: string, paramName: string, value: number, channel: number = 0): boolean {
  ...
  async playChord(channel: number, notes: number[], velocity: number = 100, durationMs: number = 500): Promise<void> {
    // Send all Note Ons
  ...
  async playScale(channel: number, rootNote: number, scale: number[], velocity: number = 80, noteDuration: number = 250, gap: number = 50): Promise<void> {
...
export function midiToPercent(value: number): number {
