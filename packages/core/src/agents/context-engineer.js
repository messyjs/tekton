...
export const DEFAULT_CONTEXT_ENGINEER_CONFIG = {
    model: "gemini-flash",
    rawWindowSize: 12,
    rewriteInterval: 10,
...
function shouldSkipForExtraction(content) {
...
export class ContextEngineer {
     ...
     */
    async extractPrecisionItems(message) {
        if (!this.llmCaller) {
...
Message (${message.role}): ${message.content}`;
        ...
        try {
            ...
            if (jsonMatch) {
                ...
                if (Array.isArray(parsed)) {
                    return parsed.filter(item => item && typeof item.category === "string" &&
                        typeof item.value === "string").map(item => ({
    ...
    addPrecisionItem(item) {
        ...
        if (lowerValue.startsWith("change ") || lowerValue.startsWith("update ") ||
            ...
            const sameCategory = this.precisionItems.filter(i => i.category === item.category && !i.superseded && !i.pinned && i.id !== item.id);
     ...
     */
    async rewriteRollingContext() {
        const rawStart = Math.max(0, this.messages.length - this.config.rawWindowSize);
        if (rawStart === 0 && this.rollingContext.length === 0)
...
//# sourceMappingURL=context-engineer.js.map