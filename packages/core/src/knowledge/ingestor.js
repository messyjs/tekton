...
export class KnowledgeIngestor {
    ...
    doclingClient;
    documents = new Map();
    constructor(config, doclingClient) {
        this.config = config;
    ...
    async ingestDirectory(dirPath) {
        ...
        for (const filePath of docsToIngest) {
            try {
                ...
                console.warn(`Failed to ingest ${filePath}: ${err.message}`);
            }
        }
        return docs;
    }
    ...
    splitIntoChunks(text, targetTokens, overlapTokens) {
