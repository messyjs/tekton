...
export class KnowledgeIndexStore {
    ...
    `);
        const transaction = this.db.transaction(() => {
            insertDoc.run(document.id, document.filename, document.title, document.filePath, document.format, JSON.stringify(document.topics), document.ingestedAt, document.totalTokens, document.fileHash);
            for (const chunk of chunks) {
                insertChunk.run(chunk.id, chunk.documentId, chunk.content, chunk.heading, chunk.pageNumber ?? null, chunk.chunkIndex, chunk.tokens, JSON.stringify(chunk.topics), chunk.embedding ? Buffer.from(new Float64Array(chunk.embedding).buffer) : null);
            }
        });
        transaction();
    }
     ...
     */
    searchByText(query, maxResults = 5) {
        const results = [];
        // Try FTS5 first
        try {
      ...
      LIMIT ?
    `).all(likeQuery, likeQuery, maxResults);
     ...
     */
    searchByTopics(topics, maxResults = 5) {
        if (topics.length === 0)
            return [];
        const results = [];
    ...
    getAllDocuments() {
        const rows = this.db.prepare("SELECT * FROM documents ORDER BY ingestedAt DESC").all();
