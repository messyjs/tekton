...
import type { PlatformName, MessageEvent, GatewaySession } from "../src/types.js";
...
describe("SessionStore", () => {
  ...
  it("stores and retrieves messages", () => {
    const key = { platform: "telegram" as PlatformName, userId: "111" };
  ...
  it("increments message count", () => {
    const key = { platform: "slack" as PlatformName, userId: "222" };
    store.getOrCreateSession(key);
  ...
  it("deletes a session", () => {
    const key = { platform: "telegram" as const, userId: "del" };
    store.getOrCreateSession(key);
  ...
  it("limits recent messages", () => {
    const key = { platform: "telegram" as const, userId: "lim" };
    store.getOrCreateSession(key);
    for (let i = 0; i < 100; i++) {
...
class TestAdapter extends BaseAdapter {
  readonly name: PlatformName = "webhook";
