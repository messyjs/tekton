...
export class DatasetPipeline {
    ...
    async splitDataset(name, trainRatio = 0.9, seed = 42, outputDir) {
    ...
    async tokenizeDataset(name, tokenizer, maxSeqLength = 2048) {
        const info = this.datasets.get(name);
        if (!info)
            throw new Error(`Dataset not found: ${name}`);
    ...
    async shardDataset(name, shardsPerFile = 10000) {
        const info = this.datasets.get(name);
        if (!info)
            throw new Error(`Dataset not found: ${name}`);
    ...
    async convertFormat(name, targetFormat, outputDir) {
        const info = this.datasets.get(name);
        if (!info)
            throw new Error(`Dataset not found: ${name}`);
    ...
    // ── Private ──────────────────────────────────────────────────────────
    async downloadHuggingFace(config, dir) {
...
`;
    ...
    async analyzeDataset(config, dir) {
    ...
    generateSplitScript(info, ratio, seed, outDir) {
...
`;
    ...
    generateTokenizeScript(info, tokenizer, maxSeqLen, outDir) {
        return `#!/usr/bin/env python3
"""Tokenize dataset."""
...
`;
    ...
    generateConvertScript(info, format, outDir) {
        return `#!/usr/bin/env python3
...
//# sourceMappingURL=dataset-pipeline.js.map