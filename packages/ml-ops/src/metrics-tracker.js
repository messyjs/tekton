...
export class MetricsTracker {
    ...
    recordMetric(jobId, point) {
        const full = { ...point, timestamp: Date.now() };
        // In-memory
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(jobId, point.step, point.epoch, point.trainLoss, point.evalLoss ?? null, point.learningRate, point.gpuUtil, point.gpuVramMB, point.samplesPerSecond, full.timestamp);
    }
    /** Parse a training log line and extract metrics */
    parseLogLine(jobId, line) {
    ...
    getMetrics(jobId) {
        ...
        const rows = this.db.prepare("SELECT * FROM metrics WHERE jobId = ? ORDER BY step ASC").all(jobId);
        return rows.map(row => ({
            step: row.step,
            epoch: row.epoch,
