...
export function parseTrainingPrompt(prompt) {
    const spec = {};
    ...
    if (epochMatch)
        spec.epochs = Number(epochMatch[1]);
    // Precision detection
    ...
    if (vramMatch)
        spec.vramBudgetMB = Number(vramMatch[1]) * 1024;
    return spec;
}
/** Build a TrainingConfig from a TrainingSpec */
export function buildTrainingConfig(spec) {
    ...
    if (spec.baseModel)
        config.baseModel = spec.baseModel;
    if (spec.dataset)
        config.dataset = spec.dataset;
    if (spec.outputName)
        config.outputName = spec.outputName;
    if (spec.epochs)
...
function inferMethod(spec) {
    if (spec.prompt) {
        const lower = spec.prompt.toLowerCase();
        if (lower.includes("reasoning") || lower.includes("grpo"))
            return "grpo";
        if (lower.includes("ternary") || lower.includes("bitnet"))
            return "ternary-bitnet";
...
function adjustForVram(config, budgetMB) {
